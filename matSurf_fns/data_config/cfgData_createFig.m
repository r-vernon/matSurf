...
% panel
opacPan = uipanel(h.confDataFig,'Tag','opacPan','Title','1. Global opacity',...
...
% transparency text
[~] = uicontrol(opacPan,'Style','text','String','Transparency (0:1)',...
...
% panel
cmapPan = uipanel(h.confDataFig,'Tag','cmapPan','Title','2. Colormap',...
...
h.addNegCM = uicontrol(cmapPan,'Style','checkbox','String',...
    'Add -ve colormap','Tag','addNegCM','Position',[10 170 200 25]);
...
h.mapCol = uicontrol(h.colLimBG,'Style','radiobutton','String',...
    'Map to nearest color','Tag','mapCol','Position',[10 40 200 25]);

% clip
h.clipCol = uicontrol(h.colLimBG,'Style','radiobutton','String',...
    'Clip (make transparent)','Tag','clipCol','Position',[10 10 200 25]);
...
sP(2) = uicontrol(statsPan,'String','Mean: ',     'Tag','meanTxt','Position',[10 135 80 15]);
sP(3) = uicontrol(statsPan,'String','Min (-ve): ','Tag','nMinTxt','Position',[10 105 80 15]);
sP(4) = uicontrol(statsPan,'String','Max (-ve): ','Tag','nMaxTxt','Position',[10 75 80 15]);
sP(5) = uicontrol(statsPan,'String','Other: ',    'Tag','othTxt', 'Position',[10 45 80 15]);
sP(6) = uicontrol(statsPan,'String','Std: ',      'Tag','sdTxt',  'Position',[190 135 80 15]);
sP(7) = uicontrol(statsPan,'String','Max (+ve): ','Tag','pMaxTxt','Position',[190 105 80 15]);
sP(8) = uicontrol(statsPan,'String','Min (+ve): ','Tag','pMinTxt','Position',[190 75 80 15]);
...
meanPan = uipanel(statsPan,'Tag','meanPan','Units','pixels','BorderType','beveledin','Position',[90 130 90 25]);
nMinPan = uipanel(statsPan,'Tag','nMinPan','Units','pixels','BorderType','beveledin','Position',[90 100 90 25]);
nMaxPan = uipanel(statsPan,'Tag','nMaxPan','Units','pixels','BorderType','beveledin','Position',[90 70 90 25]);
othPan =  uipanel(statsPan,'Tag','othPan', 'Units','pixels','BorderType','beveledin','Position',[270 40 90 25]);
sdPan =   uipanel(statsPan,'Tag','sdPan',  'Units','pixels','BorderType','beveledin','Position',[270 130 90 25]);
pMaxPan = uipanel(statsPan,'Tag','pMaxPan','Units','pixels','BorderType','beveledin','Position',[270 100 90 25]);
pMinPan = uipanel(statsPan,'Tag','pMinPan','Units','pixels','BorderType','beveledin','Position',[270 70 90 25]);

%------------------
% stats edit labels

% various text options
h.meanVal = uicontrol(meanPan,'Tag','meanVal');
h.nMinVal = uicontrol(nMinPan,'Tag','nMinVal');
h.nMaxVal = uicontrol(nMaxPan,'Tag','nMaxVal');
h.othVal =  uicontrol(othPan,'Tag','othVal');
h.sdVal =   uicontrol(sdPan,'Tag','sdVal');
h.pMaxVal = uicontrol(pMaxPan,'Tag','pMaxVal');
h.pMinVal = uicontrol(pMinPan,'Tag','pMinVal');

% set constant properties
statsGrp = [h.meanVal;h.nMinVal;h.nMaxVal;h.othVal;h.sdVal;h.pMaxVal;h.pMinVal];
set(statsGrp,'Style','text','String','','FontSize',10,'Position',[5 5 80 15],...
...
histXLimPan = uipanel(h.confDataFig,'Tag','histXLimPan','Title',...
    'Histogram','Units','pixels','Position',[925,15,100,185]);
...
h.histXLim1Edit = uicontrol(histXLimPan,'Style','edit','String','',...
    'Tag','histXLim1Edit','Position',[10 115 80 25],'UIContextMenu',h.cpMenu);
h.histXLim2Edit = uicontrol(histXLimPan,'Style','edit','String','',...
    'Tag','histXLim2Edit','Position',[10 85 80 25],'UIContextMenu',h.cpMenu);

h.histXLog = uicontrol(histXLimPan,'Style','checkbox',...
    'String','Use log','Tag','histXLog','Position',[10 55 80 25]);
...
threshPan = uipanel(h.confDataFig,'Tag','threshPan','Title',...
    '3. Thresholding (local opacity)','Units','pixels',...
...
h.sThr = uicontrol(h.thrChBG,'Style','radiobutton','String',...
    'Significance map','Tag','sThr','Position',[225 5 150 25],...
    'Enable','off');
...
h.thrRngPan = uipanel(threshPan,'Tag','thrRngPan','Title',...
    'Threshold range (1 - Visible, 0 - Hidden)','Units','pixels',...
    'Position',[90,185,430,90]);

% text labels
h.xLt_txt = uicontrol(h.thrRngPan,'String','? if x <=',...
    'Tag','xLt_txt','Position',[10 45 65 15],'Enable','off');
h.xGt_txt = uicontrol(h.thrRngPan,'String','? if x >=',...
    'Tag','xGt_txt','Position',[165 45 65 15],'Enable','off');
h.xBt1_txt = uicontrol(h.thrRngPan,'String','? if',...
    'Tag','xBt1_txt','Position',[45 15 30 15],'Enable','off');
h.xBt2_txt = uicontrol(h.thrRngPan,'String',' <= x <= ',...
    'Tag','xBt2_txt','Position',[155 15 75 15],'Enable','off');

% set constant properties
set([h.xLt_txt,h.xGt_txt,h.xBt1_txt,h.xBt2_txt],'Style','text','FontSize',10);
set([h.xLt_txt,h.xGt_txt,h.xBt1_txt],'HorizontalAlignment','left');

% edit buttons
h.xLt_edit = uicontrol(h.thrRngPan,'Style','edit','String','',...
    'Tag','xLt_edit','Position',[75 40 80 25],'Enable','off');
h.xGt_edit = uicontrol(h.thrRngPan,'Style','edit','String','',...
    'Tag','xGt_edit','Position',[230 40 80 25],'Enable','off');
h.xBt1_edit = uicontrol(h.thrRngPan,'Style','edit','String','',...
    'Tag','xBt1_edit','Position',[75 10 80 25],'Enable','off');
h.xBt2_edit = uicontrol(h.thrRngPan,'Style','edit','String','',...
    'Tag','xBt2_edit','Position',[230 10 80 25],'Enable','off');

% update button
h.updateBut = uicontrol(h.thrRngPan,'Style','pushbutton','String','Update',...
    'Tag','updateBut','Position',[330,25,80,25]);
...
thrXLimPan = uipanel(h.confDataFig,'Tag','thrAxLimPan','Title',...
    'Thr. Graph','Units','pixels','Position',[925,250,100,135]);
...
h.thrXLim1Edit = uicontrol(thrXLimPan,'Style','edit','String','',...
    'Tag','thrXLim1Edit','Position',[10 65 80 25],'UIContextMenu',h.cpMenu);
h.thrXLim2Edit = uicontrol(thrXLimPan,'Style','edit','String','',...
    'Tag','thrXLim2Edit','Position',[10 35 80 25],'UIContextMenu',h.cpMenu);

h.thrXLog = uicontrol(thrXLimPan,'Style','checkbox',...
    'String','Use log','Tag','thrXLog','Position',[10 5 80 25]);
...
% add colormap, plus stats edit textboxes to that, as can't change those
clickObj = [clickObj; h.pCmAx; statsGrp];
